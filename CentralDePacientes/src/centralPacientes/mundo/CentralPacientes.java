...
public class CentralPacientes {
    ...
    public String metodo4() {
        String cmaspacientes = null;
        int numpacientes=0;
        for(int i = 0; i< listaClinicas.size();i++){
            String clinica = listaClinicas.get(i);
            if(pacientesxClinica(clinica)>numpacientes){
                numpacientes=pacientesxClinica(clinica);
                cmaspacientes=clinica;
            }
        }
        return cmaspacientes;

    }
    private int pacientesxClinica(String nombreClinica){
        int contador=0;
        for(int i = 0; i<pacientes.size();i++){
            Paciente paciente = pacientes.get(i);
            if(nombreClinica.equals(paciente.darClinica())){
                contador++;
            }
        }
        return contador;
